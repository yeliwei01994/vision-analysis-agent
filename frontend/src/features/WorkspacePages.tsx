...
import type { EventRule, VideoJob } from '../types/events';
...
export function RulesPage({ rules, onSaved }: { rules: EventRule[]; onSaved: () => Promise<void> }) {
  const [drafts, setDrafts] = useState<Record<string, EventRule>>(() => Object.fromEntries(rules.map((rule) => [rule.event_type, { ...rule }])));
  const [saving, setSaving] = useState('');
  function update(eventType: string, field: 'min_confidence' | 'min_duration_ms', value: number) { setDrafts((current) => ({ ...current, [eventType]: { ...current[eventType], [field]: value } })); }
  ...
  return <section className="page-panel"><div className="page-heading"><div><p className="eyebrow">EVENT RULES</p><h2>规则配置</h2><p>调整检测置信度和事件持续时间阈值。</p></div></div><div className="rule-grid">{rules.length === 0 ? <div className="empty">暂无规则</div> : rules.map((rule) => { const draft = drafts[rule.event_type] ?? rule; return <div className="rule-card" key={rule.event_type}><div><strong>{rule.event_type}</strong><small>目标：{rule.class_name} · {rule.version}</small></div><label>最低置信度<input type="number" min="0" max="1" step="0.01" value={draft.min_confidence} onChange={(event) => update(rule.event_type, 'min_confidence', Number(event.target.value))} /></label><label>最短持续时间（毫秒）<input type="number" min="0" step="100" value={draft.min_duration_ms} onChange={(event) => update(rule.event_type, 'min_duration_ms', Number(event.target.value))} /></label><button className="confirm" onClick={() => save(draft)} disabled={saving === rule.event_type}>{saving === rule.event_type ? '保存中…' : '保存规则'}</button></div>; })}</div></section>;
}
