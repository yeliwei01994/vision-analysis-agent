...
export interface VideoJob { id: string; filename: string; duration_ms: number; status: string; progress: number; source_uri?: string | null; updated_at?: string | number; annotated_video_url?: string | null; annotated_video_status?: 'pending' | 'ready' | 'failed' | null; annotated_video_error?: string | null; }
...
export interface JobProgressEvent { job_id: string; status: 'pending' | 'processing' | 'completed' | 'failed' | 'cancelled'; stage?: JobStage; progress: number | null; message?: string; updated_at?: string | number; estimated_remaining_ms?: number | null; sequence: number; }
